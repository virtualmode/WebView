...
namespace WebView
{
    public partial class RichTextEditor : BaseComponent, IDisposable
    {
        ...
        public RichTextEditor()
        {
            buttonCommand = new RelayCommand(async (p) =>
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        if (!item.Checked)
                            await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, s.ToLowerInvariant());
                        else
                            await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, s.ToLowerInvariant(), false);
                    ...
                    else
                    {
                        switch (item.Key)
                        {
                            case "Image":
                                ...
                                //await jsRuntime.InvokeVoidAsync("window.richTextEditor.setFormat", quillId, "image", "";
                                break;
                        }
        ...
        protected override async Task OnParametersSetAsync()
        {
            if (_renderedOnce)
            {
                if (RichText != internalRichText)
                    await JSRuntime!.InvokeVoidAsync("richTextEditor.setHtmlContent", quillId, RichText!);
                if (ReadOnly && !_readonlySet)
                {
                    await JSRuntime!.InvokeVoidAsync("richTextEditor.setReadonly", quillId, true);
                    _readonlySet = true;
                }
                else if (!ReadOnly && _readonlySet)
                {
                    await JSRuntime!.InvokeVoidAsync("richTextEditor.setReadonly", quillId, false);
                    _readonlySet = false;
                }
            }
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                ...
                quillId = await JSRuntime!.InvokeAsync<int>("richTextEditor.register", RootElementReference, selfReference);
                if (RichText != null)
                    await JSRuntime!.InvokeVoidAsync("richTextEditor.setHtmlContent", quillId, RichText);
                if (ReadOnly)
                {
                    await JSRuntime!.InvokeVoidAsync("window.richTextEditor.setReadonly", quillId, true);
                    _readonlySet = true;
                }
                _renderedOnce = true;
        ...
        {
            FormattingState? formatState = await JSRuntime!.InvokeAsync<FormattingState>("richTextEditor.getFormat", quillId);
            if (formatState != null)
            {
                bool stateNeedsChanging = false;
        ...
        private async Task InterceptKeyPressAsync(KeyboardEventArgs keyboardEventArgs)
        {
            if (keyboardEventArgs.CtrlKey && keyboardEventArgs.Key == "+")
            {
                ...
                {
                    if (!item.Checked)
                        await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, "superscript");
                    else
                        await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, "superscript", false);
            ...
            else if (keyboardEventArgs.CtrlKey && keyboardEventArgs.Key == "=")
            {
                ...
                {
                    if (!item.Checked)
                        await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, "subscript");
                    else
                        await JSRuntime!.InvokeVoidAsync("richTextEditor.setFormat", quillId, "subscript", false);
        ...
        {
            await JSRuntime!.InvokeVoidAsync("richTextEditor.preventZoomEnable", true);
        }

        private async Task OnBlurAsync()
        {
            await JSRuntime!.InvokeVoidAsync("richTextEditor.preventZoomEnable", false);
        }
